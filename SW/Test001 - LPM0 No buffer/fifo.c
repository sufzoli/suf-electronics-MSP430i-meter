...
unsigned int fifo_read(fifo_t * f, void * buf, int nbytes)
{
     int i;
     char * p;
     p = buf;
...
unsigned int fifo_readchar(fifo_t * f, char *value)
{
    if( f->tail != f->head )  //see if any data is available
    {
         *value = f->buf[f->tail];  //grab a byte from the buffer
         f->tail++;  //increment the tail
         if( f->tail == f->size ) //check for wrap-around
         {
              f->tail = 0;
...
unsigned int fifo_write(fifo_t * f, const void * buf, int nbytes){
     int i;
     const char * p;
     p = buf;
     for(i=0; i < nbytes; i++)
